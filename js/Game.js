class Game {
  ...
  start() {
    ...
    bike3 = createSprite(width / 2 - 50, height - 100);
    bike3.addImage("bike3", bike3_img);
    bike3.scale = 0.07;

    bike4 = createSprite(width / 2 - 50, height - 100);
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        if (index === player.index) {
          ...
          this.handlecarACollisionWithCabB(index);
  ...
  handlecarACollisionWithCabB(index) {
    if(index === 1){
    if (bikes[index - 1].collide(cars[1])) {
      if (this.leftKeyActive) {
        player.positionX += 100;
      } else {
        player.positionX -= 100;
